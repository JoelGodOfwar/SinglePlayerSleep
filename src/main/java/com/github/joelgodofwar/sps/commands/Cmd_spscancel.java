	...
	public boolean execute(CommandSender sender, String[] args) {
		try {
			...
			if(Perms.CANCEL.hasPermissionOrOp(sender)) {
				if(Perms.CANCEL.hasPermission(sender)){LOGGER.debug("command Can " + sender.getName() + " has sps.cancel");}
				if(Perms.OP.hasPermission(sender)){LOGGER.debug("command Can " + sender.getName() + " has sps.op");}
					...
					if (!plugin.getConfig().getBoolean("unrestricteddayskipper")) {
						...
						if ( plugin.isDSrunning() || plugin.isDSqueued() ) {
							...
							if (!(plugin.getConfig().getBoolean("cancelbroadcast", false) == false)) {
								...
								damsg = damsg.replace("cancelmsg", get("sps.message.dayskipcanceled"));
								// nickname parser
								String nickName = plugin.getNickname(sender);
								String playercolor = "";
								if(!nickName.contains("§")){
									//logWarn("nickName ! contain SS");
									playercolor = ChatColorUtils.setColorsByName(Objects.requireNonNull(plugin.getConfig().getString("playernamecolor")));
								}else{
				...
				if (plugin.IsNight(world)||world.hasStorm()) {
					...
					if (!plugin.getConfig().getBoolean("unrestrictedsleep")) {
						...
						if ( plugin.isSleepRunning() || plugin.isSleepQueued() ) {
							...
							if (!(plugin.getConfig().getBoolean("cancelbroadcast", false) == false)) {
								LOGGER.debug("command cancelbroadcast != false");
								String damsg = "[\"\",{\"text\":\"<player> canceled sleeping.\"}]";
								//String playercolor = ChatColorUtils.setColorsByName(getConfig().getString("playernamecolor"));
								String msgcolor1 = ChatColorUtils.setColorsByName(plugin.getConfig().getString("sleepmsgcolor", "YELLOW"));
								damsg = damsg.replace("<player> canceled sleeping.", get("sps.message.canceledsleep"));
								//** nickname parser
								String nickName = plugin.getNickname(sender);
								String playercolor = "";
								if(!nickName.contains("§")){
							...
							for (Player p: Bukkit.getOnlinePlayers()){
								...
								try {
									...
									if( inbed || player.isSleeping() ){
										...
										LOGGER.debug("command Can Player is not sleeping.");
									}
								}catch (Exception e){
								...
								if(plugin.isCanceled){
									LOGGER.debug("command Can... setting isCanceled to false");
									plugin.isCanceled = false;
								}
								return true;
								//}//
