package com.github.joelgodofwar.sps.common.error;

import com.github.joelgodofwar.sps.SinglePlayerSleep;
import com.github.joelgodofwar.sps.common.collections.ExpireHashMap;
import com.github.joelgodofwar.sps.common.error.Report.ReportBuilder;
import com.github.joelgodofwar.sps.common.reflect.PrettyPrinter;
...
public class DetailedErrorReporter implements ErrorReporter {
	...
	public DetailedErrorReporter(Plugin plugin, String prefix, String supportURL, int maxErrorCount, Logger logger) {
		...
		this.logger = logger;
		this.sps = (SinglePlayerSleep) plugin;
	}
	...
	public void reportDetailed(Object sender, Report report) {
		...
		// And plugin
		if (plugin != null) {
			writer.println("Version: " + sps.getName() + " v" + sps.getDescription().getVersion());
			String jarfilename = sps.getjarfilename();
		...
		FileConfiguration config = sps.getConfig();
	...
	public static String getStringDescription(Object value) {
		...
		try {
			return PrettyPrinter.printObject(value, value.getClass(), Object.class, PrettyPrinter.RECURSE_DEPTH);
		} catch (IllegalAccessException e) {
			getBukkitLogger().log(Level.WARNING, "Cannot access object fields: " + e.getMessage());
			return "[Error: " + e.getMessage() + "]";
		} catch (ThreadDeath | OutOfMemoryError e) {
			throw e;
		} catch (Throwable e) {
			getBukkitLogger().log(Level.WARNING, "Unexpected error converting object to string: " + e.getMessage());
			return "[Error: " + e.getMessage() + "]";
		}
	}
